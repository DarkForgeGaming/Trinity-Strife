	...
	while ( true )
	{
		...
		case PKT_SC_LOGIN:
			{
				...
				if ( g_RecvBuffer.Read((char*)&recvData, header.mSize) )
				{
					...
					char buff[128] = {0, } ;
					sprintf_s(buff, "LOGIN SUCCESS ClientId[%d] Name[%s] POS(%.4f, %.4f, %.4f) \n", g_MyClientId, recvData.mName, recvData.mPosX, recvData.mPosY, 0) ;
		...
		case PKT_SC_MOVE:
		{
			...
			if (g_RecvBuffer.Read((char*)&recvData, header.mSize))
			{
				...
				char buff[128] = { 0, };
				sprintf_s(buff, "ClientId[%d] MOVE FROM (%.4f / %.4f) To (%.4f / %.4f)\n", recvData.mPlayerId, recvData.mStartPosX, recvData.mStartPosY, recvData.mEndPosX, recvData.mEndPosY);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case WM_TIMER:
		{
			...
			sendData.mPosX = (rand() % 100000) / 100000.f * 100.f;
			sendData.mPosY = (rand() % 100000) / 100000.f * 100.f;
			...
			case FD_CONNECT:
				{
					...
					std::string name = std::string("hi_") + std::to_string(rand() % 10000);
					memset(sendData.mCharacterName, 0, sizeof(sendData.mCharacterName));
					memcpy(sendData.mCharacterName, name.c_str(), name.length());
